...
# variable manipulation including loops, conditionals, and, procedures. All variables 
...
ifStatementDictionary = {} #dictionary to store if statements blocks
whileStatementDictionary = {} #dictionary to store while statements blocks
...
def Main():
	...
	Parsing(inputList)
	Iteration(inputList, 0, len(inputList))
...
def ProcedureDeclarationCheck(inputList, index):
	...
	if inputList[index] != "procedure":
		...
		procedureIdentifier = inputList[index+1]
		procedureStart = index + 3
		index = BlockCheck(inputList, index+3)
		procedureIdentifierDictionary[procedureIdentifier] = procedureStart, index
	return index
	...
	if inputList[index] != "if":
		...
		if inputList[index] != "then":
			...
			if inputList[index] == "else":
				...
				index = StatementCheck(inputList, index+1)
				elseEnd = index
				ifStatementDictionary[ifStart] = thenStart, thenEnd, elseStart, elseEnd
			else:
				ifStatementDictionary[ifStart] = thenStart, thenEnd
	return index
...
def WhileStatementCheck(inputList, index):
	...
	if inputList[index] != "while":
		...
		if inputList[index] != "do":
			...
			index = StatementCheck(inputList, index+1)
			doEnd = index
			whileStatementDictionary[whileStart] = doStart, doEnd
	return index

#Handles iteration (read, write, assignment, calling procedure)
#Also handles if, while, and procedure
def Iteration(inputList, lowerLimit, upperLimit):
	location = lowerLimit
	while location < upperLimit:
		...
		if location in ifStatementDictionary:
			if inputList[location] == "if":
				ifStart = location + 1
				ifEnd = ifStatementDictionary[location][0]
				thenStart = ifStatementDictionary[location][0] + 1
				thenEnd = ifStatementDictionary[location][1] - 1
				ifResult = ExpressionEvaluation(inputList[ifStart:ifEnd])
				if (PythonBooleanValue(ifResult)):
					Iteration(inputList, thenStart, thenEnd)
					if len(ifStatementDictionary[location]) == 4:
						location = ifStatementDictionary[location][3] - 1
					else:
						location = thenEnd - 1
				elif (PythonBooleanValue(ifResult)) & (len(ifStatementDictionary[location]) == 4):
					elseStart = ifStatementDictionary[location][2]
					elseEnd = ifStatementDictionary[location][3]
					Iteration(inputList, elseStart, elseEnd)
					location = ifStatementDictionary[location][3] - 1
				else:
					if len(ifStatementDictionary[location]) == 4:
						location = ifStatementDictionary[location][3] - 1
						...
						location = thenEnd - 1
		if location in whileStatementDictionary:
			if inputList[location] == "while":
				whileStart = location + 1
				whileEnd = whileStatementDictionary[location][0]
				doStart = whileStatementDictionary[location][0] + 2
				doEnd = whileStatementDictionary[location][1] - 1
				while (PythonBooleanValue(ExpressionEvaluation(inputList[whileStart:whileEnd]))):
					Iteration(inputList, doStart, doEnd)
		if location-2 >= lowerLimit:
			if inputList[location-2] in procedureIdentifierDictionary:
				if (procedureIdentifierDictionary[inputList[location-2]][0] == location):
					location = procedureIdentifierDictionary[inputList[location-2]][1] - 1
		if inputList[location] in procedureIdentifierDictionary:
			if inputList[location - 1] != "procedure":
				startIndex = procedureIdentifierDictionary[inputList[location]][0]
				endIndex = procedureIdentifierDictionary[inputList[location]][1]
				Iteration(inputList, startIndex, endIndex)
		...
		#user can enter expression as input value
			if inputList[location+2] in integerDictionary:
				...
				if isNumber(float(readValue)) == False:
					print("Invalid assignment: non-integer to integer")
					quit()
				else:
					integerDictionary[inputList[location+2]] = int(float(readValue))
				...
				if isNumber(float(arrayIndex)):
					arrayIndex = float(arrayIndex)
		...
		if inputList[location] == ":=":
			...
			if inputList[location-1] in integerDictionary:
				...
				if isNumber(float(result)) == False:
					print("Invalid assignment: non-integer to integer")
					quit()
				else:
					integerDictionary[inputList[location - 1]] = int(float(result))
					...
					if isNumber(float(arrayIndex)):
						...
						if (int(arrayIndex) >= len(integerArrayDictionary[inputList[i-1]])) | (int(arrayIndex) < 0):
							...
							if isNumber(float(result)) == False:
								...
								integerArrayDictionary[inputList[i-1]][int(arrayIndex)] = int(float(result))
...
def ExpressionEvaluation(currentExpression):
	...
	while index < len(currentExpression):
		...
		index = index + 1
	index = 0
	# parse and compute multiplication/division
	# precedence is left-to-right
	# float to allow for decimals in the middle of calculations
	while index < len(currentExpression):
		if currentExpression[index] == "*":
			if (isNumber((float(currentExpression[index - 1]))) &
				isNumber((float(currentExpression[index + 1])))):
				currentExpression[index - 1] = float(currentExpression[index - 1]) * float(currentExpression[index + 1])
		...
		index = index + 1
	index = 0
	# parse and compute addition/subtraction
	# precedence is left-to-right
	# float to allow for decimals in the middle of calculations
	while index < len(currentExpression):
		if currentExpression[index] == "+":
	...
	while index < len(currentExpression):
		if currentExpression[index] == "=":
			...
			currentExpression[index - 1] = SFAscalBooleanValue(float(currentExpression[index - 1]) == float(currentExpression[index + 1]))
			del currentExpression[index] # removes =
